...
local function ItemsToItemInfo(crafttype)
    ...
    elseif crafttype == 'ammo' then
        local itemInfos = {
            ...
            [6] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 36x, " ..QBCore.Shared.Items["steel"]["label"] .. ": 24x, "..QBCore.Shared.Items["aluminum"]["label"] .. ": 28x."},
            [7] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 32x, " ..QBCore.Shared.Items["steel"]["label"] .. ": 43x, "..QBCore.Shared.Items["plastic"]["label"] .. ": 61x."},
            [8] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 36x, " ..QBCore.Shared.Items["steel"]["label"] .. ": 24x, "..QBCore.Shared.Items["aluminum"]["label"] .. ": 28x."},
            [9] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 32x, " ..QBCore.Shared.Items["steel"]["label"] .. ": 43x, "..QBCore.Shared.Items["plastic"]["label"] .. ": 61x."},
        }
        return itemInfos
    end	
    return {}
...
---@return table items
local function GetThresholdItems(crafttype)
    ...
    elseif crafttype == 'weapon' then
        itemlistDetail = Config.WeaponTableItems  
    elseif crafttype == 'ammo' then
        itemlistDetail = Config.AmmoTableItems  
    end

	local itemInfos = ItemsToItemInfo(crafttype)

	local items = {}
	for _, item in pairs(itemlistDetail) do
		local itemInfo = QBCore.Shared.Items[item.name:lower()]
		items[item.slot] = {
			name = itemInfo["name"],
			amount = tonumber(item.amount),
    ...
    -- --     if PlayerData.metadata["craftingrep"] >= Config.CraftingItems[k].threshold then
		craftingitems[k] = Config.CraftingItems[k]
	-- -- 	end
	end
...
--- Craft types
RegisterNetEvent('inventory:client:proceedaction',function(building)
    local crafting = {}
    crafting.label = Lang:t("label.craft")
    print('inventory:client:proceedaction>', building)
    crafting.items = GetThresholdItems(building)
    TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
end)


RegisterNetEvent('inventory:client:craftTarget',function()
    TriggerEvent("qb-mechanicjob:client:validBuilding","craft", "Engineer")
end)
...
RegisterNetEvent('inventory:client:garageTarget',function()
    TriggerEvent("qb-mechanicjob:client:validBuilding","garage","Engineer")
end)

RegisterNetEvent('inventory:client:weaponTarget',function()
    TriggerEvent("qb-mechanicjob:client:validBuilding","weapon","Engineer")
end)

RegisterNetEvent('inventory:client:ammoTarget',function()
    TriggerEvent("qb-mechanicjob:client:validBuilding","ammo","Engineer")
end)

RegisterNetEvent('inventory:client:recycleTarget',function()
    TriggerEvent("qb-mechanicjob:client:validBuilding","recycle","Engineer")
end)

RegisterNetEvent('inventory:client:radarTarget',function()
    TriggerEvent("qb-mechanicjob:client:validBuilding","radar","Engineer")
end)
                ...
                if craftObject ~= 0 then
                    ...
                    if #(pos - objectPos) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            crafting = {}
                            crafting.label = Lang:t("label.craft")
                            crafting.items = GetThresholdItems("craft")
                            TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
                            sleep = 100
                        end
                    end
                ...
                if craftObject ~= 0 then
                    ...
                    if #(pos - objectPos) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            crafting = {}
                            crafting.label = Lang:t("label.craft")
                            crafting.items = GetThresholdItems("garage")
                            TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
                            sleep = 100
                        end
                    end
                ...
                if craftObject ~= 0 then
                    ...
                    if #(pos - objectPos) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            crafting = {}
                            crafting.label = Lang:t("label.craft")
                            crafting.items = GetThresholdItems("weaponcraft")
                            TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
                            sleep = 100
                        end
                    end
                ...
                if craftObject ~= 0 then
                    ...
                    if #(pos - objectPos) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            crafting = {}
                            crafting.label = Lang:t("label.craft")
                            crafting.items = GetThresholdItems("ammocraft")
                            TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
                            sleep = 100
                        end
                    end
                ...
                if craftObject ~= 0 then
                    ...
                    if #(pos - objectPos) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            crafting = {}
                            crafting.label = Lang:t("label.craft")
                            crafting.items = GetThresholdItems("recycle")
                            TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
                            sleep = 100
                        end
                    end
