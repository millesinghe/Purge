...
QBCore.Functions.CreateUseableItem('snp_ammo', function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if (Player.PlayerData.job.grade.name == 'Soldier') then
        TriggerClientEvent('weapons:client:AddAmmo', source, 'AMMO_SNIPER', 1, item)
    else
        TriggerClientEvent('QBCore:Notify', source, "You are not trained as a Soldier", 'error')
    end
end)
...
QBCore.Functions.CreateUseableItem('granadeluancher_ammo', function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if (Player.PlayerData.job.grade.name == 'Soldier') then
