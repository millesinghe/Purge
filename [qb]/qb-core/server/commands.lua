...
QBCore.Commands.Add('role', Lang:t("command.job.help"), {}, false, function(source)
...
QBCore.Commands.Add('team', Lang:t("command.team.help"), {}, false, function(source)
    local mygang = QBCore.Functions.GetPlayer(source).PlayerData.gang
    -- TriggerClientEvent('QBCore:Notify', source, Lang:t('info.job_info', {value = PlayerJob.label, value2 = PlayerJob.grade.name, value3 = PlayerJob.onduty}))
    TriggerClientEvent('QBCore:Notify', source, Lang:t('info.gang_info', {value = mygang}))
end, 'user')


QBCore.Commands.Add('setrole', Lang:t("command.playermanage.setrole.help"), { { name = Lang:t("command.playermanage.setrole.params.id.name"), help = Lang:t("command.playermanage.setrole.params.id.help") }, { name = Lang:t("command.playermanage.setrole.params.grade.name"), help = Lang:t("command.playermanage.setrole.params.grade.help") } }, true, function(source, args)
    local leader = QBCore.Functions.GetPlayer(source)
    if leader.PlayerData.job.grade.name == 'Leader' then
    
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_Leader'), 'error')
    end
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
        Player.Functions.SetJob('team', tostring(args[2]))
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_online'), 'error')
    end
end, 'user')
...
QBCore.Commands.Add('setteam', "create a team (Only for Leader)", { { name = "TeamName", help = "Name for the team" } }, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
        if Player.PlayerData.job.grade.name == 'Leader' then
            local cid = tostring(Player.PlayerData.citizenid)
            local result1 = MySQL.query.await('SELECT name, bosscid FROM purge_team WHERE name = ? or bosscid = ?', {tostring(args[1]), cid})          
            if result1[1] ~= nil and tostring(result1[1].teamLeader) == cid then
                TriggerClientEvent('QBCore:Notify', source, "You already owns a team", 'error')
            elseif result1[1] ~= nil and tostring(result1[1].teamName) == tostring(args[1]) then
               TriggerClientEvent('QBCore:Notify', source, "A team already registered with this name", 'error')
            else
               Player.Functions.SetGang(args[1])
               MySQL.insert('INSERT INTO purge_team (name, bosscid) VALUES (?, ?)', {args[1], Player.PlayerData.citizenid})    
            end    
        else
            TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_Leader'), 'error')
        end
...
QBCore.Commands.Add('joinplayer', Lang:t("command.playermanage.join.help"), { { name = Lang:t("command.playermanage.join.params.id.name"), help = Lang:t("command.playermanage.join.params.id.help") } }, true, function(source, args)
    local leader = QBCore.Functions.GetPlayer(source)
    if leader.PlayerData.job.grade.name == 'Leader' then
...
QBCore.Commands.Add('kickplayer', Lang:t("command.playermanage.kick.help"), { { name = Lang:t("command.playermanage.kick.params.id.name"), help = Lang:t("command.playermanage.kick.params.id.help") } }, true, function(source, args)
    local leader = QBCore.Functions.GetPlayer(source)
    if leader.PlayerData.job.grade.name == 'Leader' then
